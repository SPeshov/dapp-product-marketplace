...
contract('Marketplace', ([deployer, seller, buyer]) => {
  ...
  describe('deployment', async () => {
    ...
    it('has a name', async () => {
      ...
      assert.equal(name, 'SPeshov MarketPlace');
    });
  });
  ...
  describe('products', async () => {
    ...
    it('it creates products', async () => {
      //SUCCESS
      assert.equal(productCount, 1);

      const event = result.logs[0].args;
      assert.equal(event.name, 'Chair', 'name is correct');
      ...
      await await marketplace.createProduct('Chair', 0, { from: seller }).should
        .be.rejected;
    });
    ...
    it('it lists products', async () => {
      ...
      assert.equal(product.name, 'Chair', 'name is correct');
